...
class Kvaser:
    ...
    def mkdata(self, data):
        if isinstance(data, str):
            ...
            # 이미 bytes 형식인 경우 그대로 반환
            return bytearray(data)
        else:
            raise ValueError("Unsupported data format. Must be a hexadecimal string or bytes.")
...
def main():
    ...
    try:
        while True:
            transmitter.transmit_data(id=123, data=bytearray(b'\xff\x00\xff\x00\xff\x00\xff\x00'))
            time.sleep(0.2)
